...
modal-train-scorer-v5-antipattern-qwen3: setup-modal
	$(MODAL) run deploy/modal/studio_train_scorer_v5_antipattern.py --out-dir $(V5_SCORER_OUT_DIR_QWEN3) --bench-dir /root/horace/data/benchmarks/studio_benchmark_v5 --base-model $(V5_SCORER_BASE_MODEL) --seed $(V5_SCORER_SEED) --max-length $(V5_SCORER_MAX_LENGTH) --batch-size $(V5_SCORER_BATCH_SIZE) --lr $(V5_SCORER_LR) --weight-decay $(V5_SCORER_WEIGHT_DECAY) --epochs $(V5_SCORER_EPOCHS) --lora-r $(V5_SCORER_LORA_R) --lora-alpha $(V5_SCORER_LORA_ALPHA) --lora-dropout $(V5_SCORER_LORA_DROPOUT) --grad-accum-steps $(V5_SCORER_GRAD_ACCUM_STEPS) --eval-batch-size $(V5_SCORER_EVAL_BATCH_SIZE) $(if $(filter 1,$(V5_SCORER_GRADIENT_CHECKPOINTING)),--gradient-checkpointing) $(if $(filter 1,$(V5_SCORER_BF16)),--bf16) $(if $(filter 1,$(V5_SCORER_MERGE_LORA)),--merge-lora) $(if $(filter 1,$(V5_SCORER_TRUST_REMOTE_CODE)),--trust-remote-code)

modal-train-scorer-v5-antipattern-skywork: setup-modal
	$(MODAL) run deploy/modal/studio_train_scorer_v5_antipattern.py --out-dir $(V5_SCORER_OUT_DIR_SKYWORK) --bench-dir /root/horace/data/benchmarks/studio_benchmark_v5 --base-model Skywork/Skywork-Reward-V2-Qwen3-1.7B --seed $(V5_SCORER_SEED) --max-length $(V5_SCORER_MAX_LENGTH) --batch-size $(V5_SCORER_BATCH_SIZE) --lr $(V5_SCORER_LR) --weight-decay $(V5_SCORER_WEIGHT_DECAY) --epochs $(V5_SCORER_EPOCHS) --lora-r $(V5_SCORER_LORA_R) --lora-alpha $(V5_SCORER_LORA_ALPHA) --lora-dropout $(V5_SCORER_LORA_DROPOUT) --grad-accum-steps $(V5_SCORER_GRAD_ACCUM_STEPS) --eval-batch-size $(V5_SCORER_EVAL_BATCH_SIZE) $(if $(filter 1,$(V5_SCORER_GRADIENT_CHECKPOINTING)),--gradient-checkpointing) $(if $(filter 1,$(V5_SCORER_BF16)),--bf16) $(if $(filter 1,$(V5_SCORER_MERGE_LORA)),--merge-lora) $(if $(filter 1,$(V5_SCORER_TRUST_REMOTE_CODE)),--trust-remote-code)
...
# ---- v7: Slop-resistant Preference Scorer ----
V7_PAIRS_DIR ?= data/pairs_v7
V7_MODEL_DIR ?= models/preference_v7
V7_FEATURE_MODEL_ID ?= Qwen/Qwen3-1.7B
V7_HARD_PAIR_FILES ?= data/antipattern/hard_v7/pairs_temp03_besteff.jsonl data/antipattern/hard_v7/pairs_temp05.jsonl data/antipattern/hard_v7/pairs_temp07.jsonl
V7_ALL_PAIR_INPUT_FILES ?= $(V6_PAIR_INPUT_FILES) $(V7_HARD_PAIR_FILES)
V7_L2_REG ?= 1.0

v7-consolidate-pairs: setup
	$(PYTHON) -m tools.studio.consolidate_pairs $(foreach p,$(V7_ALL_PAIR_INPUT_FILES),--input $(p)) --out-dir $(V7_PAIRS_DIR)

modal-v7-featurize: setup-modal v7-consolidate-pairs
	$(MODAL) run deploy/modal/studio_featurize_pairs_v6.py --pairs-dir $(V7_PAIRS_DIR) --model-id $(V7_FEATURE_MODEL_ID)

v7-train:
	$(PYTHON) -m tools.studio.train_preference_features --train $(V7_PAIRS_DIR)/train_featurized.jsonl --val $(V7_PAIRS_DIR)/val_featurized.jsonl --test $(V7_PAIRS_DIR)/test_featurized.jsonl --out-dir $(V7_MODEL_DIR) --l2-reg $(V7_L2_REG)
