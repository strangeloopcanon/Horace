...
`apply_antipattern_penalty=true` is the Studio UI default, so the headline score includes authenticity adjustment. Set it to `false` if you want strict dual-score mode (quality + authenticity reported separately).
